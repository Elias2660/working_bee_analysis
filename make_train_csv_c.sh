...
echo "filename, class, beginframe, endframe"
...
function get_framecount() {
  ...
  # Read CSV file and search for the frame count
  framecount=$(awk -F, -v fname="$filename" '{
    if ($1 == fname) {
      print $2
      exit
    }
  ...
  if [[ -z $framecount ]]; then
    echo "Filename not found in CSV!"
    return 1
  else
    echo "$framecount"
  fi
...
for video in $target/*.h264; do
    ...
    vframes=$(get_framecount "${video:2}" "counts.csv")
    ...
    while [[ $cur_switch_index -lt ${#all_state_switches[@]} ]] && [[ $prev_sw_frame -lt $vframes ]]; do
        ...
        swframe=$((swframe <= vframes ? swframe : vframes))
	#swframe=$((("($swseconds - $vseconds) * $fps" <= $vframes ) && echo "($swseconds - $vseconds) * $fps" || echo "$vframes")) | bc)
       	#swframe=$(echo "($swseconds - $vseconds) * $fps" | bc)
